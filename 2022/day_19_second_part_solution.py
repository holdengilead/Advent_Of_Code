        ...
        for robot_you_can_make in self.robots.keys():
            ...
            if min([self.robots[x] for x in self.blueprint[robot_you_can_make]]) > 0:
                ...
                time_until_you_can_make_this_robot = max(
                    ...
                    [
                        math.ceil(
                            max(quantity_required - self.inventory[material], 0)
                            / self.robots[material]
                        )
                        for material, quantity_required in self.blueprint[
                            robot_you_can_make
                        ].items()
                    ]
                )
                if (
                    self.time + time_until_you_can_make_this_robot + 1
                ) < self.time_steps:
                    state = State(
                        ...
                        inventory={
                            material: (
                                ...
                                + (time_until_you_can_make_this_robot + 1)
                                * self.robots.get(material, 0)
                            )
...
def calculate_quality_level(blueprints: tuple[Blueprint, ...]) -> None:
    quality_level = 0
    print(
        f"Commencing evaluation of {len(blueprints)} blueprint{'s' if len(blueprints) != 1 else ''}.\n"
    )
    for i, blueprint in enumerate(blueprints):
        ...
        print(
            f"and done! Score is {solution.objective}, computing that took {round(time.time() - t0, 2)} seconds."
        )
        quality_level += (i + 1) * solution.objective
    print(f"\nAll blueprints evaluated. Total quality level: {quality_level}.")


def calculate_multiplied_geodes(
    blueprints: tuple[Blueprint, ...], num_blueprints: int = 3
) -> None:
    result = 1
    print(
...
if __name__ == "__main__":
    b = read_input_file("day_19_input.txt")
    t1 = time.time()
